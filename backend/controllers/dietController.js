...
// export const logDietEntry = async (req, res, next) => {
//   try {
//     // FIX: This is the critical step.
//     // The auth0Auth middleware should populate `req.auth.payload` with the decoded JWT.
//     // The 'sub' field in the payload is the unique user ID from Auth0.
//     const userId = req.auth?.payload?.sub;

//     // If the userId is missing, it means the token was invalid or the middleware failed.
//     if (!userId) {
//       throw new AppError('Invalid Token: User identifier not found.', 401);
//     }

//     // Pass both the extracted userId and the request body (logData) to the service.
//     const log = await dietService.createLog(userId, req.body);
...
// export const getDietLogsByDate = async (req, res, next) => {
//   try {
//     const userId = req.auth?.payload?.sub;
//     if (!userId) {
//       throw new AppError('Invalid Token: User identifier not found.', 401);
//     }
//     const { date } = req.params;
//     const data = await dietService.getLogsByDate(userId, date);
...
// export const updateDietLog = async (req, res, next) => {
//   try {
//     const userId = req.auth?.payload?.sub;
//     if (!userId) {
//       throw new AppError('Invalid Token: User identifier not found.', 401);
//     }
//     // FIX: Changed from 'id' to 'logId' to match the route parameter in dietRoutes.js
//     const { logId } = req.params;
//     const updatedLog = await dietService.updateLog(userId, logId, req.body);
...
// export const deleteDietLog = async (req, res, next) => {
//   try {
//     const userId = req.auth?.payload?.sub;
//     if (!userId) {
//       throw new AppError('Invalid Token: User identifier not found.', 401);
