...
export const logDietEntry = async (req, res, next) => {
  try {
    ...
    // req.body has already been cleaned by the validator, so it only contains valid fields
...
export const updateDietLog = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    const updatedLog = await dietService.updateLog(userId, id, req.body);
...
export const deleteDietLog = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    await dietService.deleteLog(userId, id);
    res.status(204).send();
  } catch (error) {
    next(error);
  }
